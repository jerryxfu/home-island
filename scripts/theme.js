...
function buildGradient(colors) {
    return `linear-gradient(135deg, ${colors.map((color, i) => `${color} ${(i / (colors.length - 1)) * 100}%`).join(", ")})`;
}
